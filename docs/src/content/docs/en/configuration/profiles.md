...
During trial, prefer **Only opened from CCR** and always open the agent from CCR. For Claude Code, keep **Isolated CCR configuration** for complete separation. Choose **Reuse existing Claude configuration** only when you intentionally want the CCR-launched CLI to share an existing configuration directory.
