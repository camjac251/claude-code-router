...
export const appCopy: Record<ResolvedLanguage, AppCopy> = {
  en: {
    ...
    text: {
      ...
      "Limits Claude Code's model picker to these entries. Separate entries with new lines or commas. Use native aliases such as opus or fable, or configured provider selectors such as openai/model. Leave empty to keep Claude Code's existing model availability.": "Limits Claude Code's model picker to these entries. Separate entries with new lines or commas. Use native aliases such as opus or fable, or configured provider selectors such as openai/model. Leave empty to keep Claude Code's existing model availability.",
      "Claude configuration": "Claude configuration",
      "Existing Claude settings file": "Existing Claude settings file",
      "Isolated CCR configuration": "Isolated CCR configuration",
      "Reuse existing Claude configuration": "Reuse existing Claude configuration",
      "Settings file": "Settings file",
      "Uses a separate Claude configuration managed by CCR.": "Uses a separate Claude configuration managed by CCR.",
      "Uses your existing Claude configuration directory, including plugins, hooks, statusline, skills, agents, and sessions. CCR routing applies only to this CLI launch. After any managed System-default backup is restored, CCR does not read or modify the selected settings file during inherited applies. Changes made by the launched CLI remain shared.": "Uses your existing Claude configuration directory, including plugins, hooks, statusline, skills, agents, and sessions. CCR routing applies only to this CLI launch. After any managed System-default backup is restored, CCR does not read or modify the selected settings file during inherited applies. Changes made by the launched CLI remain shared.",
      "CCR keeps native default paths for ~/.claude/settings.json. Custom selections must also be named settings.json and use the file's parent directory as CLAUDE_CONFIG_DIR. CCR does not read or modify the selected file during inherited applies.": "CCR keeps native default paths for ~/.claude/settings.json. Custom selections must also be named settings.json and use the file's parent directory as CLAUDE_CONFIG_DIR. CCR does not read or modify the selected file during inherited applies.",
  ...
  zh: {
    ...
    text: {
      ...
      "Limits Claude Code's model picker to these entries. Separate entries with new lines or commas. Use native aliases such as opus or fable, or configured provider selectors such as openai/model. Leave empty to keep Claude Code's existing model availability.": "限制 Claude Code 的模型选择器只显示这些条目。可使用换行或逗号分隔。支持 opus、fable 等原生别名，也支持 openai/model 等已配置的提供商选择器。留空则保留 Claude Code 现有的模型可用性设置。",
      "Claude configuration": "Claude 配置",
      "Existing Claude settings file": "现有 Claude 设置文件",
      "Isolated CCR configuration": "CCR 隔离配置",
      "Reuse existing Claude configuration": "复用现有 Claude 配置",
      "Settings file": "设置文件",
      "Uses a separate Claude configuration managed by CCR.": "使用由 CCR 管理的独立 Claude 配置。",
      "Uses your existing Claude configuration directory, including plugins, hooks, statusline, skills, agents, and sessions. CCR routing applies only to this CLI launch. After any managed System-default backup is restored, CCR does not read or modify the selected settings file during inherited applies. Changes made by the launched CLI remain shared.": "使用现有的 Claude 配置目录，包括插件、Hooks、状态栏、Skills、Agents 和会话。CCR 路由仅作用于此次 CLI 启动。恢复任何由 CCR 管理的系统默认备份后，CCR 在应用继承配置时不会读取或修改所选设置文件。由启动的 CLI 所做的更改仍会共享。",
      "CCR keeps native default paths for ~/.claude/settings.json. Custom selections must also be named settings.json and use the file's parent directory as CLAUDE_CONFIG_DIR. CCR does not read or modify the selected file during inherited applies.": "使用 ~/.claude/settings.json 时，CCR 保留原生默认路径。自定义文件也必须命名为 settings.json，CCR 会将其父目录用作 CLAUDE_CONFIG_DIR。应用继承配置时，CCR 不会读取或修改所选文件。",
      "CLI": "CLI",
      "APP": "APP",
      "CLI & APP": "CLI 和 APP",
      "CLI command": "CLI 命令",
